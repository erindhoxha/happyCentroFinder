
var locations = {
    theFold: {
    content: `
    <div style="text-align:center; max-width:230px;">
    <img style="width:50%;" src="../img/littleGiant.png">
    <h1 class="title">The fold</h1>
    <a href="www.happycentro.com target="_blank" rel="nofollow"><h3>www.happycentro.com</h3></a>
    <p class="description"> Little Giant is a full service digitally - led agency in Auckland, New Zealand.We transform brands, engage customers and grow businesses.</p>
    <a href="www.happycentro.com/jobs rel="nofollow" >
    <p>Careers</p></a>
    <button class="goTo">Find route</button>
    </div>
    `,
    pos: { lat: -39.2273694, lng:173.85180020000007 }
  },
  //  one in rome 41.87527938233829, 12.553030471353736
    queensTown: {
    content: `
    <div style="text-align:center; max-width:230px;">
    <img style="width:50%;" src="../img/littleGiant.png">
    <h1 class="title">The fold</h1>
    <a href="www.happycentro.com target="_blank" rel="nofollow"><h3>www.happycentro.com</h3></a>
    <p class="description"> Little Giant is a full service digitally - led agency in Auckland, New Zealand.We transform brands, engage customers and grow businesses.</p>
    <a href="www.happycentro.com/jobs rel="nofollow" >
    <p>Careers</p></a>
    <button class="goTo">Find route</button>
    </div>
    `,
    pos: { lat: -44.72457424179177, lng: 168.8155284953457 }
  },
    gladEye: {
    content: `
    <div style="text-align:center; max-width:230px;">
    <img style="width:50%;" src="../img/littleGiant.png">
    <h1 class="title">Little Giant </h1>
    <a href="www.happycentro.com target="_blank" rel="nofollow"><h3>www.happycentro.com</h3></a>
    <p class="description"> Little Giant is a full service digitally - led agency in Auckland, New Zealand.We transform brands, engage customers and grow businesses.</p>
    <a href="www.happycentro.com/jobs rel="nofollow" >
    <p>Careers</p></a>
    <button class="goTo">Find route</button>
    </div>
    `,
    pos: { lat: -36.8545453302915, lng: 174.78303591970848 }
  },
    infoTauranga: {
    content: `
    <div style="text-align:center; max-width:230px;">
    <img style="width:50%;" src="../img/littleGiant.png">
    <h1 class="title">Little Giant </h1>
    <a href="www.happycentro.com target="_blank" rel="nofollow"><h3>www.happycentro.com</h3></a>
    <p class="description"> Little Giant is a full service digitally - led agency in Auckland, New Zealand.We transform brands, engage customers and grow businesses.</p>
    <a href="www.happycentro.com/jobs rel="nofollow" >
    <p>Careers</p></a>
    <button class="goTo">Find route</button>
    </div>
    `,
    pos: { lat: -37.6905593802915, lng: 176.16405071970848 }
  },
  newPlymouthNews: {
    content: `
    <div style="text-align:center; max-width:230px;">
    <img style="width:50%;" src="../img/littleGiant.png">
    <h1 class="title">Little Giant </h1>
    <a href="www.happycentro.com target="_blank" rel="nofollow"><h3>www.happycentro.com</h3></a>
    <p class="description"> Little Giant is a full service digitally - led agency in Auckland, New Zealand.We transform brands, engage customers and grow businesses.</p>
    <a href="www.happycentro.com/jobs rel="nofollow" >
    <p>Careers</p></a>
    <button class="goTo">Find route</button>
    </div>
    `,
    pos: { lat: -39.0598644802915, lng: 174.0681638197085 }
  },
  littleGiant: {
    name: 'Little Giant',
    content: `
    <div style="text-align:center; max-width:230px;">
    <img style="width:50%;" src="../img/happycentro.jpg">
    <h1 class="title">Happycentro City Centre</h1>
    <a href="www.happycentro.com target="_blank" rel="nofollow"><h3>www.happycentro.com</h3></a>
    <p class="description"> Little Giant is a full service digitally - led agency in Auckland, New Zealand.We transform brands, engage customers and grow businesses.</p>
    <a href="www.happycentro.com/jobs rel="nofollow" >
    <p>Careers</p></a>
    <button class="goTo">Find route</button>
    </div>
    `,
    pos: { lat: -36.8482384548642, lng: 174.76633429527283 }
  },
  sevenGlyphs: {
    content: `
    <div style="text-align:center; max-width:230px;">
    <img style="width:50%;" src="../img/littleGiant.png">
    <h1 class="title">Little Giant </h1>
    <a href="www.happycentro.com target="_blank" rel="nofollow"><h3>www.happycentro.com</h3></a>
    <p class="description"> Little Giant is a full service digitally - led agency in Auckland, New Zealand.We transform brands, engage customers and grow businesses.</p>
    <a href="www.happycentro.com/jobs rel="nofollow" >
    <p>Careers</p></a>
    <button class="goTo">Find route</button>
    </div>
    `,
    pos: { lat: -36.8478946802915, lng: 174.74209111970845 }
  },
  saatchi: {
    content: `
    <div style="text-align:center; max-width:230px;">
    <img style="width:50%;" src="../img/satchi.png">
    <h1 class="title">Little Giant </h1>
    <a href="www.happycentro.com target="_blank" rel="nofollow"><h3>www.happycentro.com</h3></a>
    <p class="description"> Little Giant is a full service digitally - led agency in Auckland, New Zealand.We transform brands, engage customers and grow businesses.</p>
    <a href="www.happycentro.com/jobs rel="nofollow" >
    <p>Careers</p></a>
    <button class="goTo">Find route</button>
    </div>
    `,
    pos: { lat:-41.2974335802915, lng: 174.7719019197085 }
  },
  terabyte: {
    content: `
    <div style="text-align:center; max-width:230px;">
    <img style="width:50%;" src="../img/littleGiant.png">
    <h1 class="title">Little Giant </h1>
    <a href="www.happycentro.com target="_blank" rel="nofollow"><h3>www.happycentro.com</h3></a>
    <p class="description"> Little Giant is a full service digitally - led agency in Auckland, New Zealand.We transform brands, engage customers and grow businesses.</p>
    <a href="www.happycentro.com/jobs rel="nofollow" >
    <p>Careers</p></a>
    <button class="goTo">Find route</button>
    </div>
    `,
    pos: { lat: -43.55239648029149, lng: 172.61188791970847 }
  },
  parnell: {
    pos: { lat: -36.85532372663045, lng: 174.77961434494 },
    content: '<h1 class="title">Jobs currently at: Design Agency Parnell</h1>'
  },
  mountAlbert: {
    pos: { lat: -36.88202378837097, lng: 174.71078454767417 }
  },
  eastTamakiLocation: {
    pos: { lat: -36.965515977141244, lng: 174.8658545696303 }
  },
  sydney: {
    pos: { lat: -34.764866401100655, lng: 150.71268587778923 }
  },
  remuera: {
    pos: { lat: -36.8813092651609, lng: 174.81068825039415 },
    content: `<h1 class="title">Web Design Agency - Remuera</h1>
    <img class="img-agency" src="../img/agency1.jpeg">
    <h2>Reviews: </h2>
    `
  }
};